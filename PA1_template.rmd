...
It turns out, the respective mean is `r day_step_mean`, and the median is `r day_step_median`.
...
```
The number equals to `r na_total`.
...
```
In the modified dataset, the number of rows with missing values is `r na_total_2`.
...
The calculated mean is `r day_step_mean_f` (compared with `r day_step_mean` in the case of the initial dataset), and the median is `r day_step_median_f` (compared with `r day_step_median` in the case of the initial dataset).
